 ...
 * @return array
 */
function kidspress_body_classes( $classes ) {
	...
	if( ! $options['subscription_section_enable'] ) {
		$classes[] = 'subscribe-section-disabled';
	}
	...
	if ( is_singular() || is_home() ) {
		$id = ( is_home() && ! is_front_page() ) ? get_option( 'page_for_posts' ) : get_the_id();
	  	$sidebar = get_post_meta( $id, 'kidspress-selected-sidebar', true );
...
add_filter( 'body_class', 'kidspress_body_classes' );
